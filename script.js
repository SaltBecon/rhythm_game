...
let sixRandoms = []
for (let i = 0; i < 6; i++){
    sixRandoms.push(Math.random() * 6 - 3);
}
...
function drawList(){
    ...
    ctx.rect(canvas.width / 4 - 300 + sixRandoms[0], canvas.height / 2 - 30 + sixRandoms[1], 600, 60);
    ctx.fill();
    ctx.beginPath();
    ctx.fillStyle = "#00ff00";
    ctx.rect(canvas.width / 4 - 300 + sixRandoms[2], canvas.height / 2 - 30 + sixRandoms[3], 600, 60);
    ctx.fill();
    ctx.beginPath();
    ctx.fillStyle = "#0000ff";
    ctx.rect(canvas.width / 4 - 300 + sixRandoms[4], canvas.height / 2 - 30 + sixRandoms[5], 600, 60);
    ctx.fill();
    //リスト 前のレイヤー
    ...
    ctx.fillText(musicList[((musicListIndex - 1) % musicList.length + musicList.length) % musicList.length]["title"], canvas.width / 4, canvas.height / 2 + listFontSize[1] / 3 + listFontSize[1] * (scroll - listInterval) / listFontSize[0]);
    ctx.fillText(musicList[musicListIndex]["title"], canvas.width / 4, canvas.height / 2 + listFontSize[1] / 3 + listFontSize[1] * scroll / listFontSize[0]);
    ctx.fillText(musicList[(musicListIndex + 1) % musicList.length]["title"], canvas.width / 4, canvas.height / 2 + listFontSize[1] / 3 + listFontSize[1] * (scroll + listInterval) / listFontSize[0]);
    //リスト 後ろのレイヤー
    ...
    for (let i = -1; canvas.height / 2 + listInterval * i + 11 + scroll > 0; i--){
        ...
        ctx.textAlign = "center";
        ctx.fillText(musicList[((musicListIndex + i) % musicList.length + musicList.length) % musicList.length]["title"], canvas.width / 4, canvas.height / 2 + listInterval * i + listFontSize[0] / 3 + scroll);
    }
    for (let i = 0; canvas.height / 2 + listInterval * i + 11 + scroll < canvas.height; i++){
        ...
        ctx.fillStyle = "#ffffff";
        ctx.textAlign = "center";
        ctx.fillText(musicList[(musicListIndex + i) % musicList.length]["title"], canvas.width / 4, canvas.height / 2 + listInterval * i + listFontSize[0] / 3 + scroll);
    }
}
function drawGame(){
    ctx.clearRect(0, 0, canvas.width, canvas.height);
